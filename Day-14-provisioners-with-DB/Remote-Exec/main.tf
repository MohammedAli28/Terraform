...
resource "null_resource" "remote_sql_exec" {
  ...
  connection {
    type        = "ssh"
    user        = "ec2-user"
    private_key = file("${path.module}/my-key.pem")   # Replace with your PEM file path
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "mysql -h ${aws_db_instance.mysql_rds.address} -u admin -pPassword123! app < /tmp/init.sql"
    ]
  }
